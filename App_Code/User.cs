...
public class Users : System.Security.Principal.IPrincipal
{
    ...
    {
        Users u = new Users();
        string sql = String.Format("select * from [Users] where [LoginName] = '{0}' and [Password] = '{1}'", loginname, password);
        ...
        if (r.Read())
        {
            ...
            u.LastLoginTime = Convert.ToString(r["LastLoginTime"]);
            u.Email = Convert.ToString(r["Email"]);
            u.UserName = Convert.ToString(r["UserName"]);
            u.CreateTime = Convert.ToString(r["CreateTime"]);
    ...
    {
        Users u = new Users();
        string sql = String.Format("select * from [Users] where [LoginName] = '{0}'", name);
        var r = DBHelper.INST.ExecuteSqlDR(sql);
    ...
    //用户注册
    public static int Register(string username, string password, string loginName, string email, int userType = 1)
    {
        Users u = new Users();
        string sql = String.Format(
            "insert into [Users]([LoginName],[Password],[UserName],[CreateTime],[LastLoginTime],[Email],[UserType]) values('{0}','{1}','{2}','{3}','{4}','{5}','{6}')",
            loginName,
            password,
            username,
        ...
        //用于判断是否有重复的用户名或邮箱
        string query = String.Format("select * from [Users] where ( LoginName='{0}' or Email='{1}' )",
            loginName,
            email);
    ...
    {
        string sql = String.Format("update [Users] set [LastLoginTime]='{0}' where  [LoginName] ='{1}'",
        Utils.DateTime2String(DateTime.Now),
        username);
        var con = DBHelper.INST.GetConnection();
        con.Open();
    ...
    {
        Users u = new Users();
        string sql = String.Format("update [Users] set [Password]='{0}',[UserName]='{1}',[Email]='{2}',[UserType]='{3}' where  [LoginName] ='{4}'",
      password,
      username,
      email,
      userType,
    ...
    {
        string sql = "update [Users] set UserType = " + kind + " where UserID = " + uid;
    ...
    public static bool removeFromUser(int uid)
    {
        ...
        string str = "delete from [Users] where UserID=" + uid;
        DBHelper.INST.ExecuteSql(str);
        return true;
    }
    ...
    {
        string query = String.Format("select * from [Users] where ( LoginName='{0}' and Email='{1}' )",
            loginName,
            email);
        var r = DBHelper.INST.ExecuteSqlDR(query);
        if (r.Read())
    ...
    {
        string str = "update [Users] set [Password] = '" + pass + "'" + " where Email= '" + email + "'";
    ...
    {
        string str = String.Format(
            "update [Users] set [Right]={0} where [ID]={1}",
            right,
            user.UserID
            );
        DBHelper.INST.ExecuteSql(str);
